...
'''
This script preprocesses the data created by preprocess.sh. It truncates identifiers with too
many contexts, and pads identifiers with less paths with spaces.
'''
...
def process_file(file_path, data_file_role, dataset_name, word_to_count, path_to_count, max_contexts, test_set):
    sum_total = 0
    sum_sampled = 0
    total = 0
    empty = 0
    ...
    with open(output_path, 'w') as outfile:
        with open(file_path, 'r') as file:
            for line in file:
                ...
                outfile.write(target_name + num_instances + ' ' + " ".join(contexts) + csv_padding + '\n')
                total += 1
...
if __name__ == '__main__':
    ...
    parser.add_argument("-o", "--output_name", dest="output_name",
                        ...
                        default='data')
    parser.add_argument("-test_set", "--test_set", dest="test_set", required=False, default=0)
    ...
    for data_file_path, data_role in zip([test_data_path, val_data_path, train_data_path], ['test', 'val', 'train']):
        num_examples = process_file(file_path=data_file_path, data_file_role=data_role, dataset_name=args.output_name,
                                    ...
                                    max_contexts=int(args.max_contexts), test_set=args.test_set)
        if data_role == 'train':
