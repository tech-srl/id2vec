...
def ExtractFeaturesForDir(args, dir, prefix):
    command = ['node', './TSExtractor/TSExtractor/out/main.js', '--max_path_length', str(args.max_path_length),
               '--max_path_width', str(args.max_path_width), '--dir', dir, '--test_set', str(args.test_set),
...
if __name__ == '__main__':
    ...
    parser.add_argument("-threads", "--num_threads", dest="num_threads", required=False, default=2)
    parser.add_argument("-dir", "--dir", dest="dir", required=False)
    parser.add_argument("-file", "--file", dest="file", required=False)
    parser.add_argument("-test_set", "--test_set", dest="test_set", required=False, default=0)
    parser.add_argument("-log_dir", "--log_dir", dest="log_dir", required=False, default="TSE_log")
    args = parser.parse_args()

    if args.file is not None:
        command = 'node ./TSExtractor/TSExtractor/out/main.js --max_path_length ' + \
                  str(args.max_path_length) + ' --max_path_width ' + str(args.max_path_width) + \
                  ' --file ' + args.file + ' --test_set ' + str(args.test_set)  \
